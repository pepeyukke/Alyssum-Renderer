...
public class ProgramUniforms {
	...
	private static long getCurrentTick() {
        return MINECRAFT_SHIM.getCurrentTick();
	}
	...
	public static class Builder implements DynamicLocationalUniformHolder {
		...
		public OptionalInt location(String name, UniformType type) {
			int id = GL_STATE_MANAGER.glGetUniformLocation(program, name);
			...
			if ((!locations.containsKey(id) && !uniformNames.containsKey(name))) {
				...
				IRIS_LOGGER.warn("[" + this.name + "] Duplicate uniform: " + type.toString().toLowerCase() + " " + name);
		...
		public ProgramUniforms buildUniforms() {
			...
			// need to add.
			int activeUniforms = GL_STATE_MANAGER.glGetProgrami(program, GL20C.GL_ACTIVE_UNIFORMS);
			...
			for (int index = 0; index < activeUniforms; index++) {
				...
				if (provided != null && provided != expected) {
					...
					IRIS_LOGGER.error("[" + this.name + "] Wrong uniform type for " + name + ": Iris is providing " + provided + " but the program expects " + expectedName + ". Disabling that uniform.");
