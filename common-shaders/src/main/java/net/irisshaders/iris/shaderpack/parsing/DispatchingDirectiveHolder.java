...
public class DispatchingDirectiveHolder implements DirectiveHolder {
		...
		if (type == ConstDirectiveParser.Type.BOOL) {
			...
			if (consumer != null) {
				if ("true".equals(value)) {
					...
					IRIS_LOGGER.error("Failed to process " + directive + ": " + value + " is not a valid boolean value");
				}
			...
			if (IrisLogging.ENABLE_SPAM) {
				// Only logspam in dev
				IRIS_LOGGER.info("Found potential unhandled directive: " + directive);
			}
			...
			if (consumer != null) {
				if (!value.startsWith("vec4")) {
					IRIS_LOGGER.error("Failed to process " + directive + ": value was not a valid vec4 constructor");
				}
	...
	private void typeCheckHelper(String expected, Map<String, ?> candidates, ConstDirectiveParser.ConstDirective directive) {
		if (candidates.containsKey(directive.getKey())) {
			IRIS_LOGGER.warn("Ignoring " + directive + " because it is of the wrong type, a type of " + expected + " is expected.");
		}
	}
	...
	public void acceptUniformDirective(String name, Runnable onDetected) {
		// TODO
		if (IrisLogging.ENABLE_SPAM) {
			IRIS_LOGGER.warn("Not looking for a uniform directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
	...
	public void acceptCommentStringDirective(String name, Consumer<String> consumer) {
		// TODO
		if (IrisLogging.ENABLE_SPAM) {
			IRIS_LOGGER.warn("Not looking for a comment string directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
	...
	public void acceptCommentIntDirective(String name, IntConsumer consumer) {
		// TODO
		if (IrisLogging.ENABLE_SPAM) {
			IRIS_LOGGER.warn("Not looking for a comment int directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
	...
	public void acceptCommentFloatDirective(String name, FloatConsumer consumer) {
		// TODO
		if (IrisLogging.ENABLE_SPAM) {
			IRIS_LOGGER.warn("Not looking for a comment float directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
