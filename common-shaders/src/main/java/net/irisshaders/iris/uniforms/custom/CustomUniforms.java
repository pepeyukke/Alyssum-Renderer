	...
	private CustomUniforms(CustomUniformFixedInputUniformsHolder inputHolder, Map<String, Builder.Variable> variables) {
		...
		for (Builder.Variable variable : variables.values()) {
			try {
				...
				//IRIS_LOGGER.info("Was able to resolve uniform " + variable.name + " = " + variable.expression);
			} catch (Exception e) {
				IRIS_LOGGER
	...
	public static class Builder {
		...
		public void addVariable(String type, String name, String expression, boolean isUniform) {
			if (variables.containsKey(name)) {
				IRIS_LOGGER.warn("Ignoring duplicated custom uniform name: " + name);
				return;
			}

			Type parsedType = types.get(type);
			if (parsedType == null) {
				IRIS_LOGGER.warn("Ignoring invalid uniform type: " + type + " of " + name);
				return;
			}
			...
			try {
				...
				IRIS_LOGGER.warn("Failed to parse custom variable/uniform " + name + " with expression " + expression, e);
			}
		}
		...
		public CustomUniforms build(
			...
			IRIS_LOGGER.info("Starting custom uniform resolving");
